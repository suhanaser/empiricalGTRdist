        ...
        while True:
            ...
            if time.time() <= timeout1:
                if (len(rn1.get_leaves()) <= 2) or (len(rn2.get_leaves()) <= 2):
                    ...
                    return [tree, nodes, tips, r_nodes, r_tips]
...
# Randomly choose a tree
    tree, nodes, tips, r_nodes, r_tips = random_tree(trees)
...
if __name__ == '__main__': 
    rootDir = '/data/srh/simulations/hetero2'
    paramDist = empirical_dist(pd.read_csv('/data/srh/simulations/hetero/param_dist.csv')) #param_dist.csv contains all the empirical parameters for Q matrix, base frequencies and %invariant sites
    treesDist = branch_dist('/data/srh/simulations/hetero/branchLen.csv') #BranchLen.csv contains the empirical branch lengths from all trees
    ...
    simulatedTrees20taxa = '/data/srh/simulations/hetero/simTrees20taxa.txt' #simulated trees with 20 taxa
    simulatedTrees40taxa = '/data/srh/simulations/hetero/simTrees40taxa.txt' #simulated trees with 40 taxa
    simulatedTrees60taxa = '/data/srh/simulations/hetero/simTrees60taxa.txt' #simulated trees with 60 taxa
    simulatedTrees80taxa = '/data/srh/simulations/hetero/simTrees80taxa.txt' #simulated trees with 80 taxa
    simulatedTrees100taxa = '/data/srh/simulations/hetero/simTrees100taxa.txt' #simulated trees with 100 taxa
    ...
    for  m, trees in simulatedTrees.items():
        for n in [100,1000,10000]:
            for c in range(0,1000):
                Create_Hetero_Files(site_info_file, parameter_file, param_file_list, treeFile, trees, paramProbabilityDist)
                bashCommand = " ".join(["/home/suha/Hetero/Hetero2", treeFile, site_info_file, param_file_list, "-l", str(n), "-o", os.path.join(rootDir,'m'+str(m)+'n'+str(n)+'_'+str(c))])
